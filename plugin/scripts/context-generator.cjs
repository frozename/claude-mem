...
`+JSON.stringify(o,null,2):m=" "+this.formatData(o));let E="";if(n){let{sessionId:T,memorySessionId:O,correlationId:S,...p}=n;Object.keys(p).length>0&&(E=` {${Object.entries(p).map(([l,h])=>`${l}=${h}`).join(", ")}}`)}let g=`[${i}] [${a}] [${d}] ${u}${s}${E}${m}`;if(this.logFilePath)try{(0,C.appendFileSync)(this.logFilePath,g+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=u?`${u[1].split("/").pop()}:${u[2]}`:"unknown",E={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,E,n),o}},c=new Z;var Xt={};function kt(){return typeof __dirname<"u"?__dirname:(0,f.dirname)((0,he.fileURLToPath)(Xt.url))}var Ft=kt();function $t(){if(process.env.CLAUDE_MEM_DATA_DIR)return process.env.CLAUDE_MEM_DATA_DIR;let r=(0,f.join)((0,ee.homedir)(),".claude-mem"),e=(0,f.join)(r,"settings.json");try{if((0,P.existsSync)(e)){let{readFileSync:t}=require("fs"),s=JSON.parse(t(e,"utf-8")),n=s.env??s;if(n.CLAUDE_MEM_DATA_DIR)return n.CLAUDE_MEM_DATA_DIR}}catch{}return r}var N=$t(),y=process.env.CLAUDE_CONFIG_DIR||(0,f.join)((0,ee.homedir)(),".claude"),_s=(0,f.join)(y,"plugins","marketplaces","thedotmack"),cs=(0,f.join)(N,"archives"),us=(0,f.join)(N,"logs"),ms=(0,f.join)(N,"trash"),ps=(0,f.join)(N,"backups"),ls=(0,f.join)(N,"modes"),Es=(0,f.join)(N,"settings.json"),Oe=(0,f.join)(N,"claude-mem.db"),gs=(0,f.join)(N,"vector-db"),Pt=(0,f.join)(N,"observer-sessions"),te=(0,f.basename)(Pt),Ts=(0,f.join)(y,"settings.json"),fs=(0,f.join)(y,"commands"),Ss=(0,f.join)(y,"CLAUDE.md");function Ae(r){(0,P.mkdirSync)(r,{recursive:!0})}function Re(){return(0,f.join)(Ft,"..")}var De=require("crypto");var Ie=require("os"),Ce=D(require("path"),1);var j=require("fs"),X=D(require("path"),1),v={isWorktree:!1,worktreeName:null,parentRepoPath:null,parentProjectName:null};function Ne(r){let e=X.default.join(r,".git"),t;try{t=(0,j.statSync)(e)}catch(m){return m instanceof Error&&m.code!=="ENOENT"&&console.warn("[worktree] Unexpected error checking .git:",m),v}if(!t.isFile())return v;let s;try{s=(0,j.readFileSync)(e,"utf-8").trim()}catch(m){return console.warn("[worktree] Failed to read .git file:",m instanceof Error?m.message:String(m)),v}let n=s.match(/^gitdir:\s*(.+)$/);if(!n)return v;let i=n[1].match(/^(.+)[/\\]\.git[/\\]worktrees[/\\]([^/\\]+)$/);if(!i)return v;let a=i[1],d=X.default.basename(r),u=X.default.basename(a);return{isWorktree:!0,worktreeName:d,parentRepoPath:a,parentProjectName:u}}function Le(r){return r==="~"||r.startsWith("~/")?r.replace(/^~/,(0,Ie.homedir)()):r}function jt(r){if(!r||r.trim()==="")return c.warn("PROJECT_NAME","Empty cwd provided, using fallback",{cwd:r}),"unknown-project";let e=Le(r),t=Ce.default.basename(e);if(t===""){if(process.platform==="win32"){let n=r.match(/^([A-Z]):\\/i);if(n){let i=`drive-${n[1].toUpperCase()}`;return c.info("PROJECT_NAME","Drive root detected",{cwd:r,projectName:i}),i}}return c.warn("PROJECT_NAME","Root directory detected, using fallback",{cwd:r}),"unknown-project"}return t}function se(r){let e=jt(r);if(!r)return{primary:e,parent:null,isWorktree:!1,allProjects:[e]};let t=Le(r),s=Ne(t);if(s.isWorktree&&s.parentProjectName){let n=`${s.parentProjectName}/${e}`;return{primary:n,parent:s.parentProjectName,isWorktree:!0,allProjects:[s.parentProjectName,n]}}return{primary:e,parent:null,isWorktree:!1,allProjects:[e]}}var Ht=3e4;function H(r,e,t){return(0,De.createHash)("sha256").update([r||"",e||"",t||""].join("\0")).digest("hex").slice(0,16)}function G(r,e,t){let s=t-Ht;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}function re(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[String(e)]}catch{return[r]}}var b="claude";function Gt(r){return r.trim().toLowerCase().replace(/\s+/g,"-")}function L(r){if(!r)return b;let e=Gt(r);return e?e==="transcript"||e.includes("codex")?"codex":e.includes("cursor")?"cursor":e.includes("claude")?"claude":e.includes("gemini")?"gemini":e:b}function ye(r){let e=["claude","codex","cursor"];return[...r].sort((t,s)=>{let n=e.indexOf(t),o=e.indexOf(s);return n!==-1||o!==-1?n===-1?1:o===-1?-1:n-o:t.localeCompare(s)})}function Bt(r,e){return{customTitle:r,platformSource:e?L(e):void 0}}var B=class{db;constructor(e=Oe){e!==":memory:"&&Ae(N),this.db=new Me.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.db.run("PRAGMA journal_size_limit = 4194304"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addSessionPlatformSourceColumn(),this.addObservationModelColumns(),this.createPlansTable(),this.ensureMergedIntoProjectColumns(),this.addObservationSubagentColumns()}initializeSchema(){this.db.run(`
    ...
    `).get(e).count}createSDKSession(e,t,s,n,o){let i=new Date,a=i.getTime(),d=Bt(n,o),u=d.platformSource??b,m=this.db.prepare(`
    ...
    `).all(...u)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,u;if(e!==null){let p=`
      ...
      `,R=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let l=this.db.prepare(p).all(t,...a,s),h=this.db.prepare(R).all(t,...a,n+1);if(l.length===0&&h.length===0)return{observations:[],sessions:[],prompts:[]};d=l.length>0?l[l.length-1].created_at_epoch:t,u=h.length>0?h[h.length-1].created_at_epoch:t}catch(l){return l instanceof Error?c.error("DB","Error getting boundary timestamps",{project:o},l):c.error("DB","Error getting boundary timestamps with non-Error",{},new Error(String(l))),{observations:[],sessions:[],prompts:[]}}}let m=`
      SELECT *
      FROM observations
    ...
    `).get(e)||null}getOrCreateManualSession(e){let t=`manual-${e}`,s=`manual-content-${e}`;if(this.db.prepare("SELECT memory_session_id FROM sdk_sessions WHERE memory_session_id = ?").get(t))return t;let o=new Date;return this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, 'active')
    `).run(t,s,e,b,o.toISOString(),o.getTime()),c.info("SESSION","Created manual session",{memorySessionId:t,project:e}),t}close(){this.db.close()}importSdkSession(e){let t=this.db.prepare("SELECT id FROM sdk_sessions WHERE content_session_id = ?").get(e.content_session_id);return t?{imported:!1,id:t.id}:{imported:!0,id:this.db.prepare(`
      INSERT INTO sdk_sessions (
    ...
    `),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(27,new Date().toISOString())}registerPlan(e){let t=Date.now(),s=new Date(t).toISOString(),o=this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e.project,e.name,e.filePath,e.description||null,e.phaseCount||null,e.platformSource?L(e.platformSource):b,e.createdBySession||null,s,t);if(o.changes===0){let i=this.db.prepare("SELECT id, created_at_epoch FROM plans WHERE file_path = ?").get(e.filePath);return{id:Number(i.id),createdAtEpoch:i.created_at_epoch}}return c.info("DB","Plan registered",{id:Number(o.lastInsertRowid),project:e.project,name:e.name}),{id:Number(o.lastInsertRowid),createdAtEpoch:t}}getPlans(e,t,s=10){return t?this.db.prepare(`
